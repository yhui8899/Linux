...
**查看证书详细信息**：cfssl-certinfo -cert /opt/etcd/ssl/server.pem   
...
或者：
etcdctl --endpoints=https://192.168.83.141:2379 --ca-file=/opt/etcd/ssl/ca.pem --cert-file=/opt/etcd/ssl/server.pem --key-file=/opt/etcd/ssl/server-key.pem cluster-health
...
cat <<EOF >/opt/kubernetes/cfg/kube-apiserver       #kube-apiserver主配置文件：  
KUBE_APISERVER_OPTS="--logtostderr=true \\                  #开启错误日志，会输出到系统日志中。
--v=4 \\                                                    #日志级别
--etcd-servers=${ETCD_SERVERS} \\                           #连接etcd节点的地址
--bind-address=${MASTER_ADDRESS} \\                         #绑定当前节点的IP地址
--secure-port=6443 \\                                       #绑定的端口，监听6443和本地的8080端口
--advertise-address=${MASTER_ADDRESS} \\                    #集群通告的地址，填写当前地址IP地址即可
--allow-privileged=true \\                                  #允许授权，针对容器生效的。
--service-cluster-ip-range=10.0.0.0/24 \\                   #service集群IP范围
--enable-admission-plugins=NamespaceLifecycle,LimitRanger,ServiceAccount,ResourceQuota,NodeRestriction \\ #准入控制，启用一些高级功能。
--authorization-mode=RBAC,Node \\                       #授权模式，RBAC和Node授权
--kubelet-https=true \\                                 #启用kubelet https
--enable-bootstrap-token-auth \\        # 启用bootstrap-token认证，自动为node的kubelet颁发证书
--token-auth-file=/opt/kubernetes/cfg/token.csv \\      #token认证文件
--service-node-port-range=30000-50000 \\                #service-node的端口范围，与上面的IP范围是一类的
...
--v=4 \\
--master=${MASTER_ADDRESS}:8080 \\
--leader-elect=true \\                  #开启选举，会自动选举，相当于有了此组件有高可用
--address=127.0.0.1 \\
--service-cluster-ip-range=10.0.0.0/24 \\
--cluster-name=kubernetes \\
--cluster-signing-cert-file=/opt/kubernetes/ssl/ca.pem \\
...
--pod-infra-container-image=registry.cn-hangzhou.aliyuncs.com/google-containers/pause-amd64:3.0"  #镜像用于网络访问的
...
##### 由于k8s-dashboard初始证书有问题所以有些浏览器（如：谷歌）打不开无法访问dashboard页面所以需要自签一个证书，执行如下脚本即可生成自签证书；
