...
```
#创建一个命名空间
apiVersion: v1      #指定资源对象版本，查看API版本：kubectl api-versions
...
apiVersion: v1
metadata:
  name: nginx-configuration     #创建configmap的名字
...
#创建Deployment控制器，
apiVersion: apps/v1
kind: Deployment                    #资源对象
metadata:                           #元数据信息
  name: nginx-ingress-controller    #创建deployment的名称
  namespace: ingress-nginx          #在ingress-nginx命名空间下
  labels:                           #标签
    app.kubernetes.io/name: ingress-nginx       #定义的标签1
    app.kubernetes.io/part-of: ingress-nginx    #定义的标签2
spec:
  replicas: 1	                    #部署1个副本
  selector:		                    #标签选择器
    matchLabels:                    #匹配标签
      app.kubernetes.io/name: ingress-nginx		#匹配上面定义的标签：ingress-nginx
      app.kubernetes.io/part-of: ingress-nginx  #匹配上面定义的标签：ingress-nginx
  template:
    metadata:
      labels:
        app.kubernetes.io/name: ingress-nginx
...
如上信息可以看到已经分配到了`192.168.83.142`这个Node上，在142这个Node上会监听两个端口 即是nginx的80和443端口，与宿主机共用一个网络所以Pod中监听的哪些端口都会在宿主机上面呈现，因此创建前需确保宿主机端口未被占用，否则控制器会启动失败！
...
**tomcat 实例：**
```shell
apiVersion: apps/v1     #指定资源对象版本，查看API版本：kubectl api-versions
kind: Deployment        #资源对象是deployment
metadata:               #元数据信息
  labels:
    app: tomcat-test
  name: tomcat-test
...
spec:
  rules:
  - host: tomcat.xiaofeige.com
    http:
      paths:
      - backend:
          serviceName: java-svc
          servicePort: 8080  #容器服务端口
```
